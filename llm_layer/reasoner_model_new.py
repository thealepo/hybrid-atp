...
class Model:
    ...
    def generate_content(self , prompt , config: Optional[Dict] = None) -> str:
        ...
        if self.use_inference_endpoint and self.endpoint_url:
            ...
            response = self.client.text_generaton(
                prompt=prompt,
                model=self.model_id,
                **final_config
            )
...
class MathReasoner:
    def __init__(self , api_token , model_id: str = 'qwen2.5 #fix this' , use_inference_endpoint: bool = False , endpoint_url: Optional[str] = None):
